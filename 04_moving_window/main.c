/* animates a square on a window 
 * a change in the configuration of a window
 * uses StructureNotifyMask when it is created and
 * ConfigureNotify during the event loop */
...
int main(int argc, char *argv[])
{
	...
	at.event_mask = ExposureMask | StructureNotifyMask; // structure notify is given when the window's structure changes
	...
	do
	{
		...
		switch(e.type)
		{
			...
			case ConfigureNotify:
				...
				x += 5; y += 6; // move the x and y coordinates a bit
		}
